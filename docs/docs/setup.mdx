...
If you want to edit the vault file later, you can use the command:

```bash
ansible-vault edit group_vars/jenkinsServer/vault.yml
```
...
If you get an error related to ssh when launching the playbook, make sure that your ssh key is added to your ssh agent with the command:

```bash
ssh-add /path/to/your/private/key
```

And be sure to login at least once on the jenkins server with ssh to add it's fingerprint to your known_hosts file:

```bash
ssh root@your_jenkins_server_ip
```
...
From now, you can follow the next guide that will explain you how to use the whanos project !
